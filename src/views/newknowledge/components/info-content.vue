  ...
  <div class="info-content">
    <a-form ref="formRef" class="other-form" :model="formKnow" @submit="handleSubmit">
      ...
      <a-form-item field="jumpUrl" label="跳转路径：">
        <Mradio :radioarr="['游戏外','游戏内']" :defaultvalue="formKnow.jumpUrl" @change-radio="changeRadio" />
      </a-form-item>
      <a-form-item field="infoUrl" label="资讯跳转路径：">
        <a-input v-if="formKnow.jumpUrl === '游戏外'" v-model="formKnow.infoUrl" placeholder=" " />
        <div v-else class="info-select">
          <a-space>
      ...
      <a-form-item field="sortOrder" label="内容排序顺序：">
        <a-input-number v-model="formKnow.sortOrder" :hide-button="true" :min="0" :max="10" :style="{width:'188px',marginRight: '10px'}" placeholder=" " />
        <div>（0-10)</div>
      </a-form-item>
